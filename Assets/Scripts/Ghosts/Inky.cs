        ...
        if (_ChangeDirection || _currentDirection == Vector2.down)
        {
            ...
            foreach (Vector2 dir in _dirs)
            {
                ...
                if (CanMoveInDirection(dir))
                {
                    ...
                    if (_DistanceToCell >= Vector2.Distance(_TestPosition, 2*((Vector2)pacman.position + Player.Instance._currentDirection * 2) - (Vector2)blinky.position))
                    {
                        _futureDirection = dir;
                        _DistanceToCell = Vector2.Distance(_TestPosition, 2 * ((Vector2)pacman.position + Player.Instance._currentDirection * 2) - (Vector2)blinky.position);
                        //Debug.DrawLine(_TestPosition, (2 * ((Vector2)pacman.position + Player.Instance._currentDirection * 2) - (Vector2)blinky.position), new Color(0, 1, 1, 0.2f), 0.3f);
                    }
                }
            }
